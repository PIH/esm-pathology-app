...
const ReportComponent = () => {
      ...
      (encList) =>
        ...
        (encList.middle_name &&
          encList.middle_name
            ...
            .includes(patientName.toLowerCase())) ||
        (encList.given_name &&
          encList.given_name.toLowerCase().includes(patientName.toLowerCase()))
    );
  ...
  var rows = filteredEncList.map((encounterInfo) => {
    return {
      // rows.push({
      // patientLink: <a href={`/openmrs/patientDashboard.form?patientId=${encounterInfo.personId}`} >Link</a>,
      ...
      pathologyRequest: (
        <a
          href={`/openmrs/module/htmlformentry/htmlFormEntry.form?encounterId=${encounterInfo.encounterId}&mode=EDIT`}
        >
          {" "}
          Link{" "}
        </a>
            ...
            <option
              ...
              selected={
                encounterInfo.referralStatusObs
                  ? encounterInfo.referralStatusObs === ans.display && true
                  : false
              }
            >
              {ans.display}
      ...
      sampleDropOff: (
        <input
          type="checkbox"
          checked={encounterInfo.sampleDropoffObs ? true : false}
  ...
  const sampleDropOffChange = (encounterInfo) => {
    ...
    if (!encounterInfo.sampleDropoffObs) {
      postSampleDropoffObs(encounterInfo).then((Response) => {
        if (Response.ok) {
          const encIndex = tempEncList.current.findIndex(
  ...
  const sampleStatusChange = (targetedElem, encounterInfo) => {
    ...
    if (!encounterInfo.sampleStatusObs) {
      postSampleStatusChangeObs(targetedElem, encounterInfo).then(
        (response) => {
          if (response.ok) {
            const encIndex = tempEncList.current.findIndex(
              (enc) => enc.encounterUuid == encounterInfo.encounterUuid
            );
            tempEncList.current[encIndex].sampleStatusObs =
              targetedElem.display;
            tempEncList.current[encIndex].sampleStatusObsUuid =
              response.data.uuid;
      ...
      updateSampleStatusChangeObs(targetedElem, encounterInfo).then(
        (response) => {
          if (response.ok) {
            const encIndex = tempEncList.current.findIndex(
              (enc) => enc.encounterUuid == encounterInfo.encounterUuid
            );
            tempEncList.current[encIndex].sampleStatusObs = targetedElem
              ? targetedElem.display
              : "";
            tempEncList.current[encIndex].sampleStatusObsUuid = targetedElem
              ? response.data.uuid
              : "";
            setEncountersList(tempEncList.current);
          } else {
  ...
  const referralStatusChange = (targetedElem, encounterInfo) => {
    ...
    if (!encounterInfo.referralStatusObs) {
      postReferralStatusChangeObs(targetedElem, encounterInfo).then(
        (response) => {
          if (response.ok) {
            const encIndex = tempEncList.current.findIndex(
              (enc) => enc.encounterUuid == encounterInfo.encounterUuid
            );
            tempEncList.current[encIndex].referralStatusObs =
              targetedElem.display;
            tempEncList.current[encIndex].referralStatusObsUuid =
              response.data.uuid;
            setEncountersList(tempEncList.current);
          } else {
            setEncountersList(tempEncList.current);
      ...
      updateReferralStatusChangeObs(targetedElem, encounterInfo).then(
        (response) => {
          if (response.ok) {
            const encIndex = tempEncList.current.findIndex(
              (enc) => enc.encounterUuid == encounterInfo.encounterUuid
            );
            tempEncList.current[encIndex].referralStatusObs = targetedElem
              ? targetedElem.display
              : "";
            tempEncList.current[encIndex].referralStatusObsUuid = targetedElem
              ? response.data.uuid
              : "";
            setEncountersList(tempEncList.current);
          } else {
    ...
    <div>
      <div className={styles.filtersContainer}>
        <select
          ...
          onChange={(e) => handleSendingHospital(e.target.value)}
        >
        ...
        <select
          className={styles.dropdown}
          value={sampleStatus}
          onChange={(e) => handleSampleStatus(e.target.value)}
        >
          <option value="" selected disabled hidden>
            Sample Status
          </option>
        ...
        <select
          ...
          onChange={(e) => handleReferralStatus(e.target.value)}
        >
          <option value="" selected disabled hidden>
            Referral Status
          </option>
