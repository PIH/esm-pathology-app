...
const ReportComponent = () => {
  ...
  const filteredEncList = encountersList
    .filter(
      (encList) =>
        !sampleStatus ||
        (encList.sampleStatusObs && encList.sampleStatusObs.toLowerCase() == sampleStatus.toLowerCase()),
    )
    .filter(
      (encList) =>
        !referralStatus ||
        (encList.referralStatusObs && encList.referralStatusObs.toLowerCase() == referralStatus.toLowerCase()),
    )
    .filter(
      (encList) =>
        !sendingHospital ||
        (encList.patientHealthCenter && encList.patientHealthCenter.toLowerCase() == sendingHospital.toLowerCase()),
    )
    .filter(
      (encList) =>
  ...
  const rows = filteredEncList.map((encounterInfo) => {
    return {
      ...
      sampleStatus: (
        <select onChange={(e) => sampleStatusChange(e.target.value && JSON.parse(e.target.value), encounterInfo)}>
          <option value=""></option>
      ...
      referralStatus: (
        <select onChange={(e) => referralStatusChange(e.target.value && JSON.parse(e.target.value), encounterInfo)}>
